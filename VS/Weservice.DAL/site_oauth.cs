...
namespace Weservice.DAL
{
    ...
    public partial class site_oauth
    {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from " + databaseprefix + "site_oauth");
            strSql.Append(" where ");
            strSql.Append(" id = @id  ");
        ...
        public int Add(Model.site_oauth model)
        {
            ...
            strSql.Append("insert into  " + databaseprefix + "site_oauth(");
        ...
        public bool Update(Model.site_oauth model)
        {
            ...
            PropertyInfo[] pros = model.GetType().GetProperties();
            List<SqlParameter> paras = new List<SqlParameter>();
            strSql.Append("update  " + databaseprefix + "site_oauth set ");
            foreach (PropertyInfo pi in pros)
            {
                //如果不是主键则追加sql字符串
                if (!pi.Name.Equals("id"))
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from  " + databaseprefix + "site_oauth ");
            strSql.Append(" where id=@id ");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        public Model.site_oauth GetModel(int id)
        {
            ...
            strSql.Append(" from " + databaseprefix + "site_oauth");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update " + databaseprefix + "site_oauth set " + strValue);
            strSql.Append(" where id=" + id);
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.site_oauth DataRowToModel(DataRow row)
        {
            Model.site_oauth model = new Model.site_oauth();
            if (row != null)
            {
