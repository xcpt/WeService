...
namespace Weservice.Web.admin.channel
{
    public partial class attribute_field_list : Web.UI.ManagePage
    {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("attribute_field_page_size", "WeservicePage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("attribute_field_page_size", "WeservicePage", _pagesize.ToString(), 43200);
                }
            }
