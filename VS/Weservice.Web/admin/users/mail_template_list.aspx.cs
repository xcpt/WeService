...
namespace Weservice.Web.admin.users
{
    public partial class mail_template_list : Web.UI.ManagePage
    {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("mail_template_page_size", "WeservicePage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("mail_template_page_size", "WeservicePage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            ChkAdminLevel("user_mail_template", DTEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.mail_template bll = new BLL.mail_template();
            ...
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！", Utils.CombUrlTxt("mail_template_list.aspx", "keywords={0}", this.keywords));
        }
