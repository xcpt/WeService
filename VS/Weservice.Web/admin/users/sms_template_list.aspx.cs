...
namespace Weservice.Web.admin.users
{
    public partial class sms_template_list : Web.UI.ManagePage
    {
        ...
        private void RptBind(string _strWhere, string _orderby)
        {
            ...
            string pageUrl = Utils.CombUrlTxt("sms_template_list.aspx", "keywords={0}&page={1}", this.keywords, "__id__");
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("sms_template_page_size", "WeservicePage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("sms_template_page_size", "WeservicePage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("sms_template_list.aspx", "keywords={0}", this.keywords));
        }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！", Utils.CombUrlTxt("sms_template_list.aspx", "keywords={0}", this.keywords));
        }
