...
namespace Weservice.Web.admin.users
{
    public partial class recharge_list : Web.UI.ManagePage
    {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("user_recharge_log_page_size", "WeservicePage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("recharge_list.aspx", "keywords={0}", txtKeywords.Text));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("user_recharge_log_page_size", "WeservicePage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("recharge_list.aspx", "keywords={0}", this.keywords));
        }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！", Utils.CombUrlTxt("recharge_list.aspx", "keywords={0}", this.keywords));
        }
