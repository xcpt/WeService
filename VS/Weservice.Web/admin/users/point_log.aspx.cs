...
namespace Weservice.Web.admin.users
{
    public partial class point_log : Web.UI.ManagePage
    {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("user_point_log_page_size", "WeservicePage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("user_point_log_page_size", "WeservicePage", _pagesize.ToString(), 14400);
                }
            }
