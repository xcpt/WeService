...
{
    public partial class user_oauth : Web.UI.ManagePage
    {
        ...
        private void RptBind(string _strWhere, string _orderby)
        {
            ...
            string pageUrl = Utils.CombUrlTxt("user_oauth.aspx", "keywords={0}&page={1}", this.keywords, "__id__");
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("user_oauth_page_size", "WeservicePage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("user_oauth.aspx", "keywords={0}", txtKeywords.Text));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("user_oauth_page_size", "WeservicePage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("user_oauth.aspx", "keywords={0}", this.keywords));
        }
